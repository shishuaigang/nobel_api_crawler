...
class write_db:
    ...
    def write_db(self, length):
        try:  # 判定是否存在temp.csv，如没有，则抛出异常
            ...
            print u'连接数据库'
            conn = pymysql.connect(host=self.host, user=self.username, password=self.password, database=self.dbname,
                                   charset='utf8')
            cur = conn.cursor()
            print u"开始写入数据库"
            for i in range(length):  # 循环写入数据库
                sql = 'INSERT INTO nobelTestResult(TestNo, API_URL, API_ChineseName, Response_code, Response_time, ' \
                      'Response_status, Error_Message) VALUES(' + self.t + ',\'' + url[i] + '\',\'' + cnname[i] + '\',' + \
